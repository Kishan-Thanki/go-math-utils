...
func EulerTotient[T Integer](n T) (T, error) {
	if n <= 0 {
		...
		return zero, errors.New("euler totient is only defined for positive integers, got " + nStr)
	}
	...
	for i := uint64(1); i <= ua/i; i++ {
		if ua%i == 0 {
			...
			if uint64(T(i)) != i || T(i) < 0 {
				...
				return nil, errors.New("divisor " + iStr + " overflows type")
			}
			...
			if other != i {
				if uint64(T(other)) != other || T(other) < 0 {
					otherStr := strconv.FormatUint(other, 10)
					return nil, errors.New("divisor " + otherStr + " overflows type")
				}
		...
		if IsPrime(candidate) {
			...
			if count == n {
				if uint64(T(candidate)) != candidate || T(candidate) < 0 {
					...
					return zero, errors.New(nStr + "-th prime overflows type")
				}
